    ...
    <div className="bg-white flex flex-col">
      ...
      <header className="w-full border-b border-[#E6E6E6] relative z-50 bg-white">
        <div className="max-w-[1760px] mx-auto px-4 sm:px-6 lg:px-10 xl:px-20 2xl:px-24 h-auto md:h-[104px] grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-5 p-3">
          ...
          {/* Right Actions */}
          <div className="hidden lg:flex items-center gap-4 justify-self-end z-50">
            {/* Profile */}
            {isAuth && (
              <>
            ...
            {!isAuth && (
              <Link
                to="/login"
                className="h-11 w-20 cursor-pointer border-[1.5px] border-[#00000080]/50 rounded-md px-4 text-sm font-medium flex items-center justify-center"
              >
                Log In
              </Link>
            )}
          ...
          <button
            ...
            aria-expanded={open}
            className="lg:hidden justify-self-end relative w-10 h-10 z-50"
